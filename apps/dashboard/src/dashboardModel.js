...
export const SENSOR_COLOURS = Object.freeze({
  smoke: "#d17a22",
  temperature: "#c4473a",
  occupancy: "#276aa5",
  door: "#2f7954",
...
export function readingValueForControl(reading) {
  if (!reading) return "";
  if (reading.type !== "door") return String(reading.value);
  if (reading.value.obstructed) return "obstructed";
  return reading.value.open ? "open" : "closed";
}

export function manualReadingValue(sensorType, input) {
  if (sensorType === "door") {
    const states = {
      open: { open: true, obstructed: false },
      closed: { open: false, obstructed: false },
      obstructed: { open: false, obstructed: true },
    };
    if (!states[input]) throw new Error("Choose a valid door state");
    return states[input];
  }

  const numericValue = Number(input);
  if (!Number.isFinite(numericValue)) throw new Error("Enter a numeric sensor value");
  return numericValue;
}
